...
def createUser(login_session):
    ...
    user = User(username=login_session['username'], email=login_session['email'],
                profile_image=login_session['picture'], name=login_session['first_name'])
    session.add(user)
    session.commit()
...
def getGenres():
    ...
    return render_template('genres.html', genres=genres, login_session=login_session)
...
def createBook(genre_id):
    '''
    Return template to create new book in catalog. If user not logged in redirects to
    login page.
    '''
    genre = session.query(Genre).filter_by(id=genre_id).one()
    if 'username' not in login_session:
        ...
        if request.method == 'POST':
            newBook = Book(title=request.form['title'], author=request.form['author'], synopsis=request.form['synopsis'],
                           cover_image=request.form['cover_image'], genre_id=genre_id, user_id=login_session['user_id'])
            session.add(newBook)
            session.commit()
            flash("You added " + newBook.title + " to your library!")
            return redirect(url_for('getBook', genre_id=genre.id, book_id=newBook.id,
                                    login_session=login_session))
        else:
...
def getBook(genre_id, book_id):
    '''
    Return template to show a particular book in catalog with edit/delete options. If user not
    logged in return template without edit/delete options.
    '''
    genre = session.query(Genre).filter_by(id=genre_id).one()
    book = session.query(Book).filter_by(id=book_id).first()
    creator = getUserInfo(book.user_id)
    if 'username' not in login_session or creator.id != login_session['user_id']:
        return render_template('publicbook.html', book=book, genre=genre, login_session=login_session,
                               creator=creator)
    else:
        return render_template('book.html', book=book, creator=creator, genre=genre, login_session=login_session)

@app.route('/genres/<int:genre_id>/<int:book_id>/edit', methods=['GET', 'POST'])
def editBook(genre_id, book_id):
    '''
    Return template to edit a particular book in catalog. If user not
    logged in or user not creator of book, return alert saying user is not authorised.
    '''
    genre = session.query(Genre).filter_by(id=genre_id).one()
    bookToEdit = session.query(Book).filter_by(id=book_id).first()
    if bookToEdit.user_id != login_session['user_id'] or 'username' not in login_session:
        return """<script>function myFunction() {alert('You are not authorised to edit this book. 
                Please sign in.');} </script><body onload='myFunction()'>"""
    if request.method == 'POST':
...
@app.route('/genres/<int:genre_id>/<int:book_id>/delete', methods=['GET', 'POST'])
def deleteBook(genre_id, book_id):
    ...
    if bookToDelete.user_id != login_session['user_id'] or 'username' not in login_session:
        return """<script>function myFunction() {alert('You are not authorised to delete this book.
                Please sign in.');} </script><body onload='myFunction()'>"""
    if request.method == 'POST':
